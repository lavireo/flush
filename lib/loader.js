...
module.exports = (() =>
{
  ...
  mixin(constr.prototype, {
    ...
    load: function (type, name, clean = false)
    {
      ...
      let path = name.substr(0, name.length - ext.length);

      //
      // Create dir cache if it doesn't exist.
      if (!this.__caches.FILE[type])
      {
        this.__caches.FILE[type] = {};
      }

      //
      // Get file from cache
      if (this.__caches.FILE[type][path])
      {
        return this.__caches.FILE[type][path];
      }

      //
      // Load fresh file.
      let file  = this.__dir + '/' + type + '/' + path + ext;
      let found = false;
      if (this.exists(file))
      {
        if (ext === '.js')
        {
          try {
            this.__caches.FILE[type][path] = require(file);
            found = true;
          }
          catch (e)
          {
        ...
        {
          this.__caches.FILE[type][path] = fs.readFileSync(file, 'utf-8');
          found = true;
        }
      }
      ...
      if (!found)
      {
        throw new Error('Unable to find ' + type + '/' + path + ext);
      }
